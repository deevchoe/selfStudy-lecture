// 6가지의 요소 조작 메서드

// 1. push
// 배열의 맨 뒤에 새로운 요소를 추가하는 메소드
let arr1 = [1, 2, 3];
arr1.push(4);
const newLength = arr1.push(5, 6, 7); // 여러 개 추가도 가능

console.log(arr1);
console.log(newLength); // push 메소드 수행 이후에 변경된 배열의 길이인 7이 출력

// 2. pop 메소드
// 배열의 맨 뒤에 있는 요소를 제거하고, 반환
let arr2 = [1, 2, 3];
const poppedItem = arr2.pop();

console.log(poppedItem); // 3
console.log(arr2); // 1, 2

// 3. shift
// 배열의 맨 앞에 있는 요소를 제거, 반환
let arr3 = [1, 2, 3];
const shiftedItem = arr3.shift();
console.log(shiftedItem, arr3); // 1       2, 3

// 4. unshift
// 배열의 맨 앞에 새로운 요소를 추가하는 메소드
let arr4 = [1, 2, 3];
const newLength2 = arr4.unshift(0);
console.log(newLength2, arr4); // 4      0, 1, 2, 3

// push나 pop을 사용하자. shift와 unshift가 push나 pop보단 느리게 동작하걸랑

// 5. slice
// 마치 가위처럼, 배열의 특정 범위를 잘라내서 새로운 배열로 반환
let arr5 = [1, 2, 3, 4, 5];
let sliced = arr5.slice(2, 5); // 3, 4, 5    자르기시작할 인덱스,  마지막 인덱스는 +1 해줘야해~~
console.log(sliced); // 3, 4, 5
// 원본 배열의 값은 바뀌지 않아~
console.log(arr5); // 1, 2, 3, 4, 5

let sliced2 = arr5.slice(2); // 마지막까지 잘라낼거면 두번째 인자는 생략해도돼~
console.log(sliced2); // 3, 4, 5

let sliced3 = arr5.slice(-3);
console.log(sliced3); // 3, 4, 5

// 6. concat
// 두 개의 서로 다른 배열을 이어 붙여서 새로운 배열을 반환
let arr6 = [1, 2];
let arr7 = [3, 4];

let concatedArr = arr6.concat(arr7);
console.log(concatedArr); // 1, 2, 3, 4
